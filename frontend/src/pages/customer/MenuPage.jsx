...
const MenuPage = () => {
  ...
  const { addToCart, cartItems, loadCart } = useCartStore();
  ...
  const handleAddToCart = async (item) => {
    try {
      ...
      // Check for multiple restaurant error
      if (err.response?.status === 400 && err.response?.data?.message?.includes("Cart can only contain items from one restaurant")) {
        if (window.confirm("You have items from another restaurant in your cart. Clear it to add this item?")) {
        ...
        <div ref={categoriesRef} className="flex space-x-3 mb-6 overflow-x-auto">
          {categories.map((cat, index) => (
            <button
              ...
              className={`px-4 py-2 rounded-full border text-black shadow-sm whitespace-nowrap transition-colors ${cat.value === activeCategory
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    {normalizeCategory(item.category).replace("_", " ")}
                  </p>
