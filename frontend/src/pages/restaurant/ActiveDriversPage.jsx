...
const ActiveDriversPage = () => {
  ...
  useEffect(() => {
    ...
    // If backend already auto-joins restaurant room for restaurant users, this is optional.
    // Keeping it harmless for clarity.
    socket.emit("tracking:join:restaurant", { restaurantId });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredDrivers.map((driver) => (
              ...
              >
                ...
                {liveLocations[driver._id] && (
                  <div className="mt-4 text-xs text-gray-600">
                    <p className="font-medium text-gray-700">Live location</p>
                    <p>
                      Lat: {liveLocations[driver._id].latitude?.toFixed?.(6) ?? liveLocations[driver._id].latitude}
                    </p>
                    <p>
                      Lng: {liveLocations[driver._id].longitude?.toFixed?.(6) ?? liveLocations[driver._id].longitude}
                    </p>
                  </div>
                )}
                ...
                <div className="mt-5 flex gap-3">
                  <button
                    type="button"
                    onClick={() => navigate(`/restaurant/drivers/${driver._id}/map`)}
